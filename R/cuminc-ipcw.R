...
#' #' @param model.censoring Type of regression model for the censoring distribution. "aareg" for Aalen's additive hazards model, and "coxph" for Cox's proportional hazards model.
#' #' @param formula.censoring Formula specifying the model for the censoring distribution. If NULL, uses the same mean model as for the outcome.
...
#' #'
#' #' @param formula A formula specifying the model. The left hand side must be a \link[survival]{Surv} object. The right hand side is the usual linear combination of covariates.
#' #' @param time Numeric constant specifying the time at which the effect estimates are desired.
#' #' @param cause Character constant specifying the cause indicator of interest.
...
#' #' @export
#' #'
#' rmeanglm.ipcw <- function(formula, time, cause = "1", link = "identity", data,
#'                            model.censoring = "aareg", formula.censoring = NULL, ...) {
#'
#'     outcome <- model.response(model.frame(update.formula(formula, . ~ 1), data = data))
#'     newdata <- do.call(rbind, lapply(1:length(time), function(i) data))
...
#'     }
#'
#'     if(attr(outcome, "type") == "mright") {
#'         Vi <- (time - pmin(outcome[, "time"], time)) * as.numeric(outcome[, "status"] == cause)
#'
#'     } else {
#'         Vi <- pmin(outcome[, "time"], time)
#'
#'     }
#'
#'     Ii <- as.numeric(outcome[, "time"] >= time | outcome[, "status"] != 0)
